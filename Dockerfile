FROM        buildpack-deps:jessie
MAINTAINER  Nala Ginrut
ENV         LANG C.UTF-8
ENV         GUILE_VERSION 2.1.8
RUN set -ex \
	&& echo "deb http://mirrors.ustc.edu.cn/debian testing main contrib non-free" >> /etc/apt/source.list \
	&& echo "deb-src http://mirrors.ustc.edu.cn/debian testing main contrib non-free" >> /etc/apt/source.list \
	&& apt-get update && apt-get build-dep guile-2.0 && rm -rf /var/lib/apt/lists/*
RUN set -ex \
	&& wget -c http://ftp.gnu.org/gnu/lightning/lightning-2.1.0.tar.gz \
	&& tar xvzf lightning-2.1.0.tar.gz \
	&& rm -f lightning-2.1.0.tar.gz \
	&& cd lightning-2.1.0 \
	&& ./configure && make && make install \
	\
	&& wget -c https://github.com/NalaGinrut/guile-tjit/archive/tjit-2.1.8.975-1f6fc-rebase.tar.gz \
	&& tar xvzf tjit-2.1.8.975-1f6fc-rebase.tar.gz \
	&& rm -f tjit-2.1.8.975-1f6fc-rebase.tar.gz \
	&& cd tjit-2.1.8.975-1f6fc-rebase && ./configure --enable-lightning && make \
	&& make install

CMD ["guile --tjit"]

